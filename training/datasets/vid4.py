...
def LR_DIR(s, d, scl): return VID4_DIR + "/DIV2K_" + \
    s + "_LR_" + d + "/X" + str(scl) + "/"

def HR_DIR(s): return VID4_DIR + "DIV2K_" + s + "_HR/"
...
class Vid4:
    def __init__(self, lr_shape, scale=2, subset="train", downscale="bicubic"):
        ...
        if not downscale in ['bicubic', 'unknown']:
            raise ValueError(
                'Div2k downscale method must be bicubic or unknown')
    ...
    def _is_minimum_size(self, image_id, lr_shape):
        im = Image.open(self.lr_dir + image_id +
